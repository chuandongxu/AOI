...
Solid * BooleanModeller::getDifference(bool bShowSecondObj)
{
	m_pObject2->invertInsideFaces();
	Solid * result = composeSolid(Face::OUTSIDE, Face::OPPOSITE, Face::INSIDE, bShowSecondObj);
	m_pObject2->invertInsideFaces();

	return result;
}

Solid * BooleanModeller::composeSolid(int faceStatus1, int faceStatus2, int faceStatus3, bool showSecondObj)
{
	...
	if (showSecondObj) groupObjectComponents(*m_pObject2, *vertices, *indices, *colors, faceStatus3, faceStatus3);
...
VectorSet* BooleanModeller::getFaceSplits()
{
	return m_pObject1->vectorSplits;
}
