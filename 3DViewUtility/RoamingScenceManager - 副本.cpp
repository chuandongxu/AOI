...
void RoamingScenceManager::getAxisXLimit(int& nMaxValue, int& nMinValue)
{
	nMaxValue = AXES_LEN_X / 2 + AXES_LEN_OFFSET_X;
	nMinValue = -AXES_LEN_X / 2 + AXES_LEN_OFFSET_X;
}

void RoamingScenceManager::getAxisYLimit(int& nMaxValue, int& nMinValue)
{
	nMaxValue = AXES_LEN_Y / 2 + AXES_LEN_OFFSET_Y;
	nMinValue = -AXES_LEN_Y / 2 + AXES_LEN_OFFSET_Y;
}
...
void RoamingScenceManager::setXYSize(int xAxisLen, int yAxisLen, int xOffset, int yOffset)
{
	AXES_LEN_X = xAxisLen;
	AXES_LEN_OFFSET_X = xOffset;

	AXES_LEN_Y = yAxisLen;
	AXES_LEN_OFFSET_Y = yOffset;
}
...
void RoamingScenceManager::executeTranslateOperation(int x, int y)
{
	...
	*TempTranslateVec = *TempTranslateVec + vec*0.02*AXES_LEN_X/20;
    *OldMouse=*Mouse;
...
void RoamingScenceManager::fullView(ViewPerspective viewPsp)
{
	...
	*AuxX = Vector3D::crossProduct(*AuxY, *AuxZ);
	...
	{
	case ACTION_PERSPECT_FRONT:
		TempTranslateVec->setX(-AXES_LEN_OFFSET_X);
		TempTranslateVec->setY(-AXES_LEN_OFFSET_Y);
...
void RoamingScenceManager::init_CoordinaryDisplayList()
{
	...
	double dAxisLenX = AXES_LEN_X / 2 - AXES_LEN_OFFSET_X;
	double dAxisLenY = AXES_LEN_Y / 2 - AXES_LEN_OFFSET_Y;
	double dAxisZLen = AXES_LEN_Z / 2 - AXES_LEN_Z_OFFSET;

	//确定坐标系原点
	glPushMatrix();
	glColor3f(1.0f, 1.0f, 1.0f);
	glTranslatef(AXES_LEN_OFFSET_X, AXES_LEN_OFFSET_Y, AXES_LEN_Z_OFFSET);
	glPopMatrix();

	//画网格线 Y
	glPushMatrix();
	{
		Position3D cpoint1 = Position3D(-dAxisLenX, -dAxisLenY, -dAxisZLen);
		Position3D cpoint2 = Position3D(-dAxisLenX + AXES_LEN_X, -dAxisLenY, -dAxisZLen + AXES_LEN_Z);
	...
	glPushMatrix();
	{
		...
		Position3D cpoint3 = Position3D(-dAxisLenX, 0, -dAxisLenY);
		Position3D cpoint4 = Position3D(-dAxisLenX + AXES_LEN_X, 0, -dAxisLenY + AXES_LEN_Y);
		glColor3f(0.5f, 0.5f, 0.5f);
	...
	glPushMatrix();
	{
		...
		glTranslatef(0, dAxisLenY, -0);
		Position3D cpoint5 = Position3D(-dAxisLenY, 0, -dAxisZLen);
		Position3D cpoint6 = Position3D(-dAxisLenY + AXES_LEN_Y, 0, -dAxisZLen + AXES_LEN_Z);
		glColor3f(0.5f, 0.5f, 0.5f);
	...
	glTranslatef(-dAxisLenX, -dAxisLenY, -dAxisZLen);
	...
	gluCylinder(objCylinder, 0.05, 0.05, AXES_LEN_X, 10, 5);           
	glTranslatef(0, 0, AXES_LEN_X);
	gluCylinder(objCylinder, 0.2, 0.0, 0.5, 10, 5);                 
	glPopMatrix();
	...
	glTranslatef(-dAxisLenX, -dAxisLenY, -dAxisZLen);
	glRotatef(-90, 1.0, 0.0, 0.0);
	gluCylinder(objCylinder, 0.05, 0.05, AXES_LEN_Y, 10, 5);           
	glTranslatef(0, 0, AXES_LEN_Y);
	gluCylinder(objCylinder, 0.2, 0.0, 0.5, 10, 5);                 
	glPopMatrix();
	...
	//绘制平行于Y的直线
	double dGraduationLen = AXES_LEN_X * 0.015;
	int num = AXES_GRADUATION;
	const double _xLen = AXES_LEN_X / num;
	const double _yLen = AXES_LEN_Y / num;
...
void RoamingScenceManager::render_CoordinaryDisplayText()
{
	...
	double dExtLen = (double)AXES_LEN_X / 20 * 1;

	// Axis Name
	glColor3f(0.2f, 0.2f, 0.2f);
	glPushMatrix();
	drawString(QString("Z"), -dAxisLenX, -dAxisLenY, -dAxisZLen + AXES_LEN_Z + dExtLen, axisFont);  // Print GL Text ToThe Screen
	drawString(QString(m_bXYCoordinateInvert ? "Y" : "X"), -dAxisLenX + AXES_LEN_X + dExtLen, -dAxisLenY, -dAxisZLen, axisFont); // Print GL Text ToThe Screen
	drawString(QString(m_bXYCoordinateInvert ? "X" : "Y"), -dAxisLenX, -dAxisLenY + AXES_LEN_Y + dExtLen, -dAxisZLen, axisFont); // Print GL Text ToThe Screen
	glPopMatrix();	
	...
	for (int zi = 0; zi <= num; zi++)
	{
		...
		drawString(QString::number(z / AXES_UNIT, 'g', 2), x, y - (double)AXES_LEN_X / 20 * 1, z, axisGradFont);
	}
	...
	for (int xi = 0; xi <= num; xi++)
	{
		...
		drawString(QString::number(x / AXES_UNIT, 'g', 2), x, y - (double)AXES_LEN_X / 20 * 1, z, axisGradFont);
	}
	...
	for (int yi = 0; yi <= num; yi++)
	{
		...
		drawString(QString::number(y / AXES_UNIT, 'g', 2), x - (double)AXES_LEN_X / 20 * 0.5, y, z, axisGradFont);
	}
...
void RoamingScenceManager::GLDrawSpaceAxes(void)
{
	...
	gluCylinder(objCylinder, 0.1, 0.1, AXES_LEN_X, 10, 5);         //Z
	glTranslatef(0, 0, AXES_LEN_X);
	gluCylinder(objCylinder, 0.3, 0.0, 0.6, 10, 5);                 //Z
	glPopMatrix();

	glPushMatrix();
	glTranslatef(0, 0.5, AXES_LEN_X);
	glRotatef(90, 0.0, 1.0, 0.0);
	...
	glRotatef(-90, 1.0, 0.0, 0.0);
	gluCylinder(objCylinder, 0.1, 0.1, AXES_LEN_X, 10, 5);         //Y
	glTranslatef(0, 0, AXES_LEN_X);
	gluCylinder(objCylinder, 0.3, 0.0, 0.6, 10, 5);                 //Y
	glPopMatrix();

	glPushMatrix();
	glTranslatef(0.5, AXES_LEN_X, 0);
	...
	glRotatef(90, 0.0, 1.0, 0.0);
	gluCylinder(objCylinder, 0.1, 0.1, AXES_LEN_X, 10, 5);         //X
	glTranslatef(0, 0, AXES_LEN_X);
	gluCylinder(objCylinder, 0.3, 0.0, 0.6, 10, 5);                 //X
	glPopMatrix();

	glPushMatrix();
	glTranslatef(AXES_LEN_X, 0.5, 0);
