...
void MotionControl::clearError(int AxisID)
{
    ...
    // 清除轴状态
	sRtn += GT_ClrSts((short)AxisID, (short)8);
...
void MotionControl::clearAllError()
{
    ...
    for (int i = 0; i < getMotorAxisNum(); i++)
    {
        sRtn += GT_ClrSts((short)(m_mapMtrID.values().at(i)), (short)8);
    }
...
{
    setDO(Config->ID(DO_Z_BRAKE), 1);
	QThread::msleep(100);
    setDO(Config->ID(DO_Z_BRAKE), 0);
	QThread::msleep(500);
	...
	for (int i = nMtrNum - 1; i >= 0; i--)
	{
		...
		{
			if (!home(getMotorAxisID(i), false))
			{
				return false;
			}
		}
