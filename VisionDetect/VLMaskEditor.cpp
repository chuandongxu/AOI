...
void QVLMaskEditor::repaintAll()
{
	...
	for (int i = 0; i < m_selectRects.size(); i++)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			line(matImage, vertices[i], vertices[(i + 1) % 4], Scalar(0, 255, 0), 1);
		}
	}
	...
	for (int i = 0; i < m_maskObjs.size(); i++)
	{
		...
		case MASK_TYPE_RECT:
		{
			...
			if (maskObj && !maskObj->isEmpty())
			{
				...
				cv::Mat matRect = matImage(Rect(x, y, width, height));
				//rectangle(matRect, vertices[1], vertices[3], Scalar(0,0,255, 100), -1);
				cv::Mat imgLayer(height, width, matImage.type(), maskObj->IsSelect() ? Scalar(128, 128, 255) : Scalar(0, 0, 255));
...
bool QVLMaskEditor::getRotRectLocationPt(Mat& img, int nCornerNum, Point2f* ptCorners, int nLocationNum, Point2f* ptLocations, double& dRadius)
{
	...
	double radiusImageWidth = convertToImgX(10, true);
	double radiusImageHeight = convertToImgY(10, true);

	double dCircleRadius = (radiusImageWidth + radiusImageHeight) / 2;
	...
	double dDistVt = qSqrt(qPow(ptCorners[1].x - ptCorners[3].x, 2) + qPow(ptCorners[1].y - ptCorners[3].y, 2)) / 5;

	//double radiusLensWidth = convertToImgX(50, true);
	//double radiusLensHeight = convertToImgY(50, true);
	//double dDistVt = (radiusLensWidth + radiusLensHeight) / 2;
...
void QVLMaskEditor::drawSelectRect(cv::RotatedRect& selectRect)
{
	...
	cv::Mat imgLayer(matImage.rows, matImage.cols, matImage.type(), Scalar(231, 191, 200));
	...
	if (getRotRectLocationPt(matImage, 4, vertices, 5, locations, dCircleRadius))
	{
		for (int i = 0; i < 4; i++)
		{			
			line(matImage, locations[i], locations[(i + 1) % 4], Scalar(255, 0, 0), 1);
		}
		...
		line(matImage, Point2f(XVt, YVt), Point2f(locations[4].x, locations[4].y), Scalar(255, 0, 0), 1);
