...
void SysCalibrationView::on3DDetectCali()
{
    ...
    if (m_stCalcCmds[nDLPIndex].matThickToThinK.empty() || m_stCalcCmds[nDLPIndex].matThickToThinnestK.empty())
    {
        ...
        m_stCalcCmds[nDLPIndex].enProjectDir = static_cast<Vision::PR_DIRECTION>(projectDir);
        fileNode = fs["ScanDir"];
        cv::read(fileNode, scanDir, 0);
        m_stCalcCmds[nDLPIndex].enScanDir = static_cast<Vision::PR_DIRECTION>(scanDir);
...
void SysCalibrationView::on3DDetectCaliComb()
{
    ...
    if (retStatus == Vision::VisionStatus::OK)
    {
        ...
        for (int i = 0; i < stRpy.vecHeightGridHeights.size(); ++ i)
        {
            VectorOfFloat vecDatas;
            m_dlpCaliRstData[nDLPIndex].push_back(vecDatas);

            float fTargtHeight = stRpy.vecHeightGridHeights[i].first;
            VectorOfFloat heightVec = stRpy.vecHeightGridHeights[i].second;
